...
console.log(`The diet of a velociraptor was ${raptor.diet}.`);

// How long was a stegosaurus?
console.log(`A stegosaurus was ${stego.length} long.`);
...
console.log(`The tyrannosaurus roared ${tRex.roar = "RAWERSRARARWERSARARARRRR!"}`);
...
zooAnimals.map((currValue) => {
  lowCaseAnimalNames.push(currValue.animal_name.toLowerCase());
})
...
zooAnimals.filter((currValue) => {
  if (currValue.population < 5) {
    lowPopulationAnimals.push(currValue);
  }
})
...
*/
const populationTotal = zooAnimals.reduce((accumulator, currValue) => {
  return accumulator + currValue.population;
}, 0);
